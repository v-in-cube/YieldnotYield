...
def cluster_wise_cv(gb_regressor, X, y, cluster_labels, param_grid, fp_name):
    ...
    for cluster in np.unique(cluster_labels):
        ...
        plot_model(ax, y_test, y_pred, title=f"Cluster {cluster+1}")
        plt.xlabel('True Values')
        plt.ylabel('Predicted Values')
        plt.title(f'Model Performance - Cluster {cluster}')
        plt.savefig(f"./models/cluster_{cluster+1}_fingerprint_{fp_name}_plot.png", dpi=300)
        plt.close(fig)
